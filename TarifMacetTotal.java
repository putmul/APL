public class TarifMacetTotal implements TarifStrategy {
    private final float multiplier = 1.8f; // Sesuai UML Diagram

    @Override
    public float hitungTarif(float jarak, float waktu, float baseRateJarak, float baseRateMenit) {
        System.out.println("[INFO] Menggunakan Strategy: TarifMacetTotal");
        float tarifDasar = (jarak * baseRateJarak) + (waktu * baseRateMenit);
        return tarifDasar * multiplier;
    }
}